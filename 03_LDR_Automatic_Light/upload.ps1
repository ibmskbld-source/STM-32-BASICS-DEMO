# PowerShell script to build and flash LDR Automatic Streetlight Demo to STM32G431 board via ST-LINK
...
Write-Host "[1/3] Compiling LDR Streetlight firmware..." -ForegroundColor Cyan
& $GCC -mcpu=cortex-m4 -mthumb -mfloat-abi=soft -O1 -T "$ScriptDir\linker.ld" --specs=nano.specs --specs=nosys.specs -nostartfiles -o "$ScriptDir\ldr_light.elf" "$ScriptDir\startup.c" "$ScriptDir\main_direct.c"
...
& $OBJCOPY -O binary "$ScriptDir\ldr_light.elf" "$ScriptDir\ldr_light.bin"

Write-Host "[3/3] Flashing STM32G431 via ST-LINK..." -ForegroundColor Cyan
& $CLI -c port=SWD freq=4000 mode=HOTPLUG -e all -w "$ScriptDir\ldr_light.bin" 0x08000000 -v -rst

if ($LASTEXITCODE -eq 0) {
    Write-Host "`nSUCCESS: LDR Streetlight Demo flashed and running!" -ForegroundColor Green
} else {
