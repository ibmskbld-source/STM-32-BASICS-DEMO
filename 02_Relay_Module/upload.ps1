# PowerShell script to build and flash Relay Module Demo to STM32G431 board via ST-LINK
...
Write-Host "[1/3] Compiling Relay firmware..." -ForegroundColor Cyan
& $GCC -mcpu=cortex-m4 -mthumb -mfloat-abi=soft -O1 -T "$ScriptDir\linker.ld" --specs=nano.specs --specs=nosys.specs -nostartfiles -o "$ScriptDir\relay.elf" "$ScriptDir\startup.c" "$ScriptDir\main_direct.c"
...
& $OBJCOPY -O binary "$ScriptDir\relay.elf" "$ScriptDir\relay.bin"

Write-Host "[3/3] Flashing STM32G431 via ST-LINK..." -ForegroundColor Cyan
& $CLI -c port=SWD freq=4000 mode=HOTPLUG -e all -w "$ScriptDir\relay.bin" 0x08000000 -v -rst

if ($LASTEXITCODE -eq 0) {
    Write-Host "`nSUCCESS: Relay Demo flashed and running!" -ForegroundColor Green
} else {
